...
from keras.layers import Dense, Flatten, GlobalAveragePooling1D, Dropout, Activation, TimeDistributed, LSTM, Input, Concatenate
...
"""
def load_data():
    with h5.File('trafficData3.hdf5', 'r') as f:
...
def get_model():
    ...
    auxiliary_input = Input(shape = time_data.shape, name='aux_input')
...
time_data = np.ones((537, 9, 1), dtype=int)
model = get_model()
...
model.fit({'main_input': X_train, 'aux_input': time_data},
          {'main_output': y_train},
